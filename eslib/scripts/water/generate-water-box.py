...
#---------------------------------------#
description = "Generate a structure of N water molecules at a target density, placed randomly with rotations, then analyze distances."
...
def prepare_args(description):
    ...
    parser.add_argument("-d", "--density", **argv, type=float, required=False,
                        help="Target density [g/cm^3] (default: %(default)s)", default=0.997)
    ...
    parser.add_argument("-md", "--min_dist", **argv, type=float, required=False,
...
def main(args):
    # Generate water monomer
    print("\n\tGenerating water monomer ... ", end="")
    monomer = molecule("H2O")
    print("done")

    # Compute cubic box from density
    NA = 6.02214076e23  # Avogadro
    M_water = 18.01528  # g/mol

    total_mass = args.nmol * M_water / NA  # grams
    volume_cm3 = total_mass / args.density
    volume_A3 = volume_cm3 * 1e24
    box_length = volume_A3 ** (1/3)
    box = np.array([box_length]*3)

    print(f"\tTarget density: {args.density} g/cm³")
    print(f"\tComputed cubic box: {box_length:.2f} Å per side")
