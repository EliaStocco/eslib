...
def main(args):
    ...
    assert Ns == 19, "The number of structures is not 19"
    ...
    assert len(nonzero_cols) == 9, "The number of non-zero columns is not 9"
    
    nonzero_atoms = np.unique(nonzero_cols//3).astype(np.int32)
    assert len(nonzero_atoms) == 3, "The number of non-zero atoms is not 3"
    
    tmp = np.diff(positions[:,nonzero_atoms,:],axis=0).reshape((18,9))
    assert np.all(tmp.any(axis=0)), "There is at least one column with all zero elements"
