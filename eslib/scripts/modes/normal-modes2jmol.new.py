...
from eslib.tools import convert, clean_structure
...
def main(args):
    ...
    if np.any(np.isnan(modes)):
        ...
        nm.eigvec2modes()
        modes = nm.get("mode").real
    atoms = clean_structure(nm.reference)
    ...
    #---------------------------------------#
    structures = AtomicStructures([ atoms.copy() ] * len(freqs))
