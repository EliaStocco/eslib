...
#---------------------------------------#
def circular_autocorrelation_along_axis(x: np.ndarray, axis: int) -> np.ndarray:
    """
    ...
    acorr = np.fft.ifft(power, axis=0)
    ...
    # Normalize
    m2 = np.mean(np.square(x),axis=0,keepdims=True)
    acorr /= m2    
...
def main(args):
    ...
    for n,k in enumerate(["L1","L2","L3"]):
        ...
        mean = np.mean(gdf,axis=(1,2),keepdims=True)
        gdf -= mean
        autocorr = circular_autocorrelation_along_axis(gdf, axis=2)
        autocorr = np.linalg.norm(autocorr,axis=3)/np.sqrt(3.)
