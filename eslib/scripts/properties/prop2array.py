...
description = "Save an 'info' from an *.pickle properties file to a txt file."
...
def main(args):
    ...
    if args.append:
        ...
        with eslog(f"Reading properties from file '{args.input}'"):
            atoms = Properties.from_file(file=args.input,format=args.input_format,index=args.index)
        ...
        #------------------#
        trajectories:List[Properties] = [None]*n_files
        data:List[np.ndarray] = [None]*n_files
        if args.parallel:
            #------------------#
            with eslog(f"Reading properties from '{args.input}' (parallel)"):
                def read_atomic_structure(file: str) -> Properties:
                    # with eslog(f"Reading properties from file '{file}'"):
                    return Properties.from_file(file=file,format=args.input_format,index=args.index)
            ...
            for n,file in enumerate(files):
                with eslog(f"Reading properties from file '{file}'"):
                    trajectories[n] = Properties.from_file(file=file,format=args.input_format,index=args.index)
