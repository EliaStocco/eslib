...
description = "Select a diverse subset of structures using the CUR algorithm."
...
def prepare_args(description):
    ...
    parser.add_argument("-o"  , "--output"          , type=str     , required=False, **argv, help="output file with the selected structures (default: %(default)s)", default="CUR.extxyz")
...
def main(args):
    ...
    print("\tExtracting structures using the CUR algorithm:")
    struct_idx = CUR(n_to_select=args.number, progress_bar = True, random_state=0).fit(X.T).selected_idx_
    X_CUR = X[struct_idx]

    print("\n\tCUR selected indices: {:d}".format(struct_idx.shape[0]))
    print(f"\tOriginal: {X.shape} ---> CUR: {X_CUR.shape}")
    ...
    # Saving the CUR selected structure
    ...
    if args.output is not None :
        print("\n\tSaving CUR selected structures to file '{:s}' ... ".format(args.output),end="")
        frames_CUR = frames.subsample(indices) # [frames[i] for i in indices]
        frames_CUR.to_file(file=args.output, format=args.output_format) # fmt)
        print("done")
