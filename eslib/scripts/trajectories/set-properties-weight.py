...
#---------------------------------------#
description = "Set the weight of missing properties to 0 (it needs the JSON output of 'properties2json.py')."
...
ase_like_properties = {
    ...
    "BECx": ("natoms", 3),  # ("natoms", 3, 3) is not supported by ASE
    "BECy": ("natoms", 3),  # ("natoms", 3, 3) is not supported by ASE
    "BECz": ("natoms", 3),  # ("natoms", 3, 3) is not supported by ASE
    "piezoelectric": (3, 3, 3),
}
    ...
    for atoms in structures:
        ...
        for array in all_arrays:
            if array not in atoms.arrays:
                shape = shapes[array]
                if shape[0] == "natoms":
                    shape = tuple([Natoms]+list(shape[1:]))
                v = np.full(shape,np.nan)
                atoms.arrays[array] = v
                atoms.info[f"{array}_weight"] = 0
            else:
                atoms.info[f"{array}_weight"] = 1
    print("done")
